...
class IBALayer(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self._learning_rate = tf.get_variable('learning_rate', dtype=tf.float32, initializer=0.01)
        self._beta = tf.get_variable('beta', dtype=tf.float32,
                                     initializer=np.array(4).astype(np.float32))
...
#     def call(self, inputs):
        ...
        # lamb = _gaussian_blur(lambda_pre_blur, std=self._smooth_std)
        lamb = lambda_pre_blur
        ...
        # ε ~ N(μ_r, σ_r)
        eps = std_r_min * std_normal + self._mean_r
#         lamb = std_r_min * std_normal + self._mean_r
        Z = lamb * R + (1 - lamb) * eps
...
#         output = tf.cond(self._restrict_flow, lambda: [Z_with_passing,lamb], lambda: [inputs, pass_mask])
#         score_map = tf.cond(self._restrict_flow, lambda: lamb, lambda: pass_mask)
        
        # save capacityies

        self._capacity = (_kl_div(R, lamb, self._mean_r, std_r_min) *
